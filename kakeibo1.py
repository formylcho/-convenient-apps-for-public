...
class Kakeibo:
    ...
    def add_expense(self, amount):
        ...
        st.write(f"今年の支出: {self.expense_year}円")
        st.write(f"今月の支出: {self.expense_month}円")
        st.write(f"今週の支出: {self.expense_week}円")
        st.write(f"今日の支出: {self.expense_day}円")
        # 5. 最後に、データをファイルに保存する
        self.save_data()
...
def main():
    ...
    now = datetime.datetime.now()
    st.write("家計簿アプリを起動しました！")
    st.write(f"現在の日時: {now.year}年{now.month}月{now.day}日")
    st.write(f"今年の支出: {kakeibo.expense_year}円")
    st.write(f"今月の支出: {kakeibo.expense_month}円")
    st.write(f"今週の支出: {kakeibo.expense_week}円")
    st.write(f"今日の支出: {kakeibo.expense_day}円")
    ...
    while True:
        ...
        with col1:
            pressed1 = st.button("1.支出を記録する", key="record")
        with col2:
            pressed2 = st.button("2.終了する", key="quit")
        if pressed2:
            st.write("家計簿アプリを終了します。")
            break
        elif pressed1:
            try:
                amount = int(st.text_input("いくら使用しましたか？: "))
                kakeibo.add_expense(amount)
            except ValueError:
                st.warning("数値で入力してください。")
        else:
            st.warning("無効な選択です。もう一度選んでください。")
